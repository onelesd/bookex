defmodule Bookex.API.Watcher do
  @moduledoc false
  require Logger
  ...
  defp validate("node", [script|_], merged_opts) do
    ...
    cond do
      !System.find_executable("node") ->
        Logger.error "Could not start watcher because \"node\" is not available. Your " <>
      ...
      not File.exists?(script_path) ->
        Logger.error "Could not start node watcher because script #{inspect script_path} does not " <>
                     "exist. Your application is still running, however assets " <>
